...
using CST_150_Milestone_7.BusinessLayer;
...
namespace CST_150_Milestone_7.PresentationLayer
{
    ...
    public partial class FrmMain : Form
    {
        ...
        {
            // Get the trimmed, lowercase search term from the search textbox
            string searchTerm = txtSearch.Text.Trim().ToLower();

            // If the search term is empty, show a warning message and exit the method
            ...
            // Filter the inventory list using the provided search term
            List<Inventory> filteredList = Inventory.SearchInventory(inventoryList, searchTerm);

            // Create and display a secondary form with the filtered search results
            FrmSecondary frmSecondary = new FrmSecondary(filteredList);
            frmSecondary.ShowDialog(); // Show the form as a modal dialog
        }
    }
}
